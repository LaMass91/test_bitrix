...
// Если это раздел "Обувь" (ID = 1) — убираем нужные свойства
if ($sectionId === 1) {
    $hideCodes = ['MANUFACTURER'];

    foreach ($arResult['ITEMS'] as $index => $item) {
        if (isset($item['CODE']) && in_array($item['CODE'], $hideCodes)) {
            unset($arResult['ITEMS'][$index]);
        }
    }

    // Пересобираем индексы, чтобы не было дыр (опционально, но рекомендуется)
